...
import { Power, Trash2, ExternalLink } from 'lucide-react'
...
export function AgentTable({ agents, onEnable, onDisable, onDelete }: AgentTableProps) {
  if (agents.length === 0) {
    return (
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-700">
            ...
            <th className="text-left py-3 px-4 text-primary font-medium">Port</th>
            <th className="text-left py-3 px-4 text-primary font-medium">IP</th>
            <th className="text-left py-3 px-4 text-primary font-medium">Forgejo User</th>
        ...
        <tbody>
          {agents.map((agent) => (
            <tr key={agent.name} className="border-b border-gray-800 hover:bg-dark-lighter/50">
              ...
              <td className="py-3 px-4">{agent.http_port}</td>
              <td className="py-3 px-4 font-mono text-sm">{agent.container_ip}</td>
              <td className="py-3 px-4">{agent.forgejo_username || '-'}</td>
              <td className="py-3 px-4">
