...
import { Session, Message, Process, Activity, Task, FileInfo, FileContent, FileResponse } from '@/lib/types'
import { Button } from '@/components/Button'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/Card'
import { Modal } from '@/components/Modal'
import { X, Folder, File, ChevronRight, Save, RefreshCw, Send, AlertTriangle, Clock, Terminal, ListTodo, Activity as ActivityIcon, MessageSquare, Zap, Bot } from 'lucide-react'

type TabType = 'sessions' | 'processes' | 'tasks' | 'activities' | 'files' | 'chat'
...
export default function AgentApp() {
    ...
    try {
      switch (activeTab) {
        case 'sessions':
          ...
          break
        case 'processes':
          const processesRes = await api.listProcesses()
        ...
        case 'tasks':
          ...
          break
        case 'activities':
          const activitiesRes = await api.listActivities()
  ...
  const loadSessionMessages = async (sessionId: string) => {
    const res = await api.getSessionMessages(undefined, sessionId)
    if (res.success && res.data) setMessages(res.data.messages)
  }

  const loadProcessOutput = async (processId: string) => {
    const res = await api.getProcessOutput(undefined, processId)
  ...
  const loadFiles = async (path: string) => {
    ...
    try {
      if (path) {
        const res = await api.getFile(undefined, path)
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true)
    await api.updateFile(undefined, selectedFile.path, editContent)
  ...
  const handleSendChat = async () => {
    ...
    setChatMessage('')
    if (selectedSession) {
      await api.sendSessionChat(undefined, selectedSession.id, msg)
      loadSessionMessages(selectedSession.id)
    } else {
      await api.sendMessage(undefined, msg)
    }
  }

  const handleSendInterrupt = async () => {
    if (!interruptMessage.trim() || !selectedSession) return
    await api.sendSessionInterrupt(undefined, selectedSession.id, interruptMessage)
    setInterruptMessage('')
  }

  const handleSendRemind = async () => {
    if (!remindMessage.trim()) return
    await api.sendRemind(undefined, remindMessage)
    setRemindMessage('')
  }
  ...
  if (isLoading && !sessions.length && !processes.length) {
    return (
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-primary">OpenZerg Agent</h1>
              <p className="text-gray-400 text-sm">{version && `v${version}`} - Direct Connection</p>
            </div>
          </div>
        ...
        <div className="flex gap-1 mb-4 border-b border-gray-700 pb-2 overflow-x-auto">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id); setSelectedSession(null); setSelectedProcess(null) }}
            ...
            <>
              {activeTab === 'sessions' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="border border-gray-700 rounded overflow-auto max-h-[60vh]">
                    ...
                    {sessions.length === 0 ? (
                      ...
                      sessions.map(s => (
                        <div
                          key={s.id}
                          onClick={() => { setSelectedSession(s); loadSessionMessages(s.id) }}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded ${s.state === 'Completed' ? 'bg-green-900/50 text-green-400' : s.state === 'Generating' ? 'bg-blue-900/50 text-blue-400' : s.state === 'Failed' ? 'bg-red-900/50 text-red-400' : 'bg-gray-700 text-gray-300'}`}>{s.state}</span>
                          </div>
                  ...
                  <div className="lg:col-span-2 border border-gray-700 rounded overflow-auto max-h-[60vh]">
                    ...
                    {selectedSession ? (
                      <div className="flex flex-col">
                        <div className="flex-1 overflow-auto p-3 space-y-3 max-h-[40vh]">
                          {messages.map(m => (
              ...
              {activeTab === 'processes' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="border border-gray-700 rounded overflow-auto max-h-[60vh]">
                    ...
                    {processes.length === 0 ? (
                      <div className="p-4 text-center text-gray-400 text-sm">No processes</div>
                    ) : (
                      processes.map(p => (
                  ...
                  <div className="lg:col-span-2 border border-gray-700 rounded overflow-auto max-h-[60vh]">
                    ...
                    {selectedProcess ? (
                      <pre className="p-3 text-xs font-mono whitespace-pre-wrap overflow-auto">{processOutput || 'No output'}</pre>
                    ) : (
                      <div className="p-4 text-center text-gray-400 text-sm">Select a process</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {activeTab === 'tasks' && (
                <div className="border border-gray-700 rounded overflow-auto max-h-[60vh]">
                  {tasks.length === 0 ? (
                    <div className="p-4 text-center text-gray-400">No tasks</div>
                  ) : (
              ...
              {activeTab === 'activities' && (
                <div className="border border-gray-700 rounded overflow-auto max-h-[60vh]">
                  {activities.length === 0 ? (
                    <div className="p-4 text-center text-gray-400">No activities</div>
                  ) : (
                  ...
                  {selectedFile ? (
                    ...
                    <div>
                      ...
                      <div className="flex justify-end mb-2">
                        <Button variant="secondary" size="sm" onClick={() => loadFiles(currentPath)}><RefreshCw className="w-4 h-4" /></Button>
                      </div>
                      {files.length === 0 ? (
                        <div className="text-center py-8 text-gray-400">No files</div>
                      ) : (
                        <div className="border border-gray-700 rounded max-h-[60vh] overflow-auto">
                          {files.map((file, i) => (
              ...
              {activeTab === 'chat' && (
                <div className="space-y-4 max-w-2xl">
                  ...
                  <div className="text-sm text-gray-400 p-4 bg-dark-lighter rounded">
                    <p><strong>Tip:</strong> Use Sessions tab to view conversation history.</p>
                    <p className="mt-2"><strong>Messages</strong> go through the thinking layer for task analysis.</p>
                    <p className="mt-2"><strong>Reminders</strong> are immediate notifications.</p>
                  </div>
                </div>
              )}
            </>
